...
pub fn unary<'tcx>(op: UnaryOp, ty: Type) -> fn(Slot,Slot) -> Instr<'tcx> {
    let size = 0xFFFF;
...
pub fn binary<'tcx>(op: BinaryOp, ty: Type) -> (fn(Slot,Slot,Slot) -> Instr<'tcx>,bool) {
    let size = 0xFFFF;
...
pub fn cast<'tcx>(arg_layout: Type, res_layout: Type) -> fn(Slot,Slot) -> Instr<'tcx> {
    panic!("todo fix cast");
